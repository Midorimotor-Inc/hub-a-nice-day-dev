...
(async () => {
  ...
  // m2〜: 登録済み・自分専用のスマホ
  {
    ...
    {
      const b = {};
      b.monthNav = await boxOf(pm, () => [...document.querySelectorAll('div')].find(e => e.offsetParent !== null && /^\d{4}年 ?\d{1,2}月/.test(e.textContent.trim()) && e.textContent.length < 12));
      b.carRow = await boxOf(pm, () => { const t = [...document.querySelectorAll('table')].find(x => x.offsetParent !== null); return t && t.querySelector('tbody tr td'); });
      b.rental = await boxOf(pm, byText, 'レンタカー');
