...
async function buildAdmin() {
  ...
  stepsShot('管理者コンソールの開き方', 'スケジュール画面とは別のページです。自分の端末（自分専用のPC・スマホ）から開きます。', [
    ...
    { t: '2回目からはログイン不要', d: '同じブラウザで開けば、そのまま入れます。別のブラウザ・別の窓は未登録扱いです。', color: BLUE },
  ...
  (() => {
    const s = pres.addSlide(); s.background = { color: PAPER };
    pageTitle(s, '本番とDEVの切り替え', '同じコンソールで両方を扱います。緑＝DEV（テスト環境）、青＝本番。');
    ...
    callout(s, 8.75, 5.4, 4.0, 1.7, '本番の登録は別', '本番側は最初に一度、メール → 6桁でログインが要ります。本番のスタッフ表・端末台帳・招待は、DEVとは別のデータです。', '');
  })();
