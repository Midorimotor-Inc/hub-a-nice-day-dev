...
(async () => {
  ...
  const M = { t: 'スマホ', color: BLUE }, MC = { t: 'スマホ・便利', color: ORANGE }, A = { t: 'コンソール', color: '7C3AED' }, AP = { t: 'スケジュール', color: '0F766E' };
  ...
  await annotated(M, 'タイムスケジュールの予約カード', '作業内容を選び、内容をひとこと', 'm6c-sched-edit.png', [
    { k: 'name', t: '氏名 ＊' },
    { k: 'car', t: '車種 ＊' },
  ...
  await annotated(M, '代車タブ', '代車・レンタカーの貸出状況を横に見ます（見るだけ。付けるのは予約カードから）', 'm7-loaner.png', [
    ...
    { k: 'today', t: '今日', d: '今日を先頭に戻す' },
    { k: 'honten', t: '自分の店の代車', d: '店ごとの代車の一覧' },
    { k: 'carName', t: '車の名前・番号' },
    { k: 'bar', t: '貸出の帯', d: '色の帯が貸出中（お客様名）。タップで貸出〜返却の詳細' },
    { k: 'rental', t: 'レンタカー（両店共通）' },
  ], { colW: 5.0 });
  ...
  slideCover('管理者マニュアル', 'スタッフの招待・端末の管理・復旧', '第1部 管理者コンソール／第2部 スケジュールシステムの管理者機能', '管理者のみ配布・社外秘');
  ...
  slideSection(1, '管理者コンソール', 'スタッフの招待・端末・管理者を管理する専用画面（テスト版サイトの admin.html）', ['ログイン', 'スタッフと招待', '追加・設定', '招待コード', '登録端末の取り消し', '管理者']);
  await annotated(A, 'コンソールにログインする', 'テスト版サイトの admin.html。管理者のアドレス＋招待コードで入ります', 'a1-login.png', [
    { k: 'mail', t: '管理者のメールアドレス' },
    { k: 'code', t: '招待コード（6桁）', d: '自分に発行されたコード。同じコードで別の PC にもログインできます' },
    { k: 'ok', t: 'コードでログイン' },
    { k: 'send', t: 'コードが無い時', d: 'ログイン用リンクをメールで受け取る（1日の上限あり）' },
    { k: 'env', t: 'メインに切り替える', d: 'テスト版とメインでスタッフ表は別。許可簿とアカウントは共通' },
  ...
  slideText(A, 'メールが届かない時', '2026年9月に直した内容と、それでも届かない時', [
    { n: 1, t: '迷惑メールを確認', d: '送信元は hubaniceday.system@gmail.com（GAS 経由）。件名「[テスト版] ログインの登録をお願いします」または「ログインの登録をお願いします」。' },
  ...
  slideSection(2, 'スケジュールシステムの管理者機能', 'ふだんのスケジュール画面（PC）で、管理者だけが使えるボタン', ['スタッフ設定', '休業日設定', '車検台数制限・月間休日数・7台目の承認', '復旧（バックアップから戻す）']);
  await annotated(AP, 'スケジュール画面：管理者だけのボタン', '右上の自分の名前の横', 'p1-header.png', [
  ...
  slideText(A, '運用の決まり', '2026年9月時点', [
    { t: 'メインへ招待する前に', d: 'まずテスト版で全員に使ってもらい、不具合を潰してからメイン（本番）に招待する。メインのスタッフ表はテスト版から写す（Claude 作業）。', tone: 'warn' },
